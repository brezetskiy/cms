...
class  SEO extends phpMorphy {
	...
	public function contentFromFile ($filename) {
		if (file_exists($filename) && is_readable($filename)) {
			$this->line = file_get_contents($filename);	
	...
	/**
	 * Метод формирует контент из 
	 * переданого теста
	 * 
	 * @param string $string
	 * @return string
	 */
	public function contentFromString ($string) {
		$this->line = strip_tags($string);
		return $this->line; 
	}
	 ...
	 * @param int $id
 	 ...
 	 * @return string
	 */
	public function updateDescription($lenght=250) {
		global $DB;
