...
class CVS {
	...
	/**
	* ѕроврка на наличие блокировки страницы, если страница заблокирована, то возвращает массив,
	* с указанием логина пользовател€, который заблокировал страницу и временем автоматической разблокировки.
	* ≈сли она не заблокирована, то блокирует ее. 
	* ≈сли страница заблокирована этим же пользователем, то продлевает срок жизни блокировки
	* 
	...
	*/
	...
	/**
	* ќбновление или добавление блокировки
	* @param string $table_name
	* @param int $edit_id
	* @return mixed
	*/
	public static function lock($table_name, $field_name, $edit_id) {
		...
		/**
		 * ”читыва€ тот факт, что один пользователь может редактировать только одну страницу
	...
	/**
	* ¬едет лог изменений в системе и делает проверку, надо ли сохран€ть изменени€
	* @param string $table_name
	* @param int $edit_id
	...
	*/
	public static function log($table_name, $field_name, $edit_id, $content) {
		...
		/**
		 * ќпредел€ем id текущей сессии (блокировки)
		 */
		$query = "
			SELECT id 
			FROM cvs_lock
		...
		/**
		 * ‘иксируем новое изменение в CVS
		 */
		$query = "
			INSERT INTO cvs_log
			SET
		...
		/**
		 * —охран€ем текущую версию загруженных файлов
		 */
		...
		// —охран€ем контент после того, как в нем заменили ссылки на файлы в папке uploads.
	...
	*/
	...
	/**
	 * ѕровверка наличи€ блокировки на объекте
	 *
	 * @param string $table_name
	 * @param int $id
	 * @return bool
