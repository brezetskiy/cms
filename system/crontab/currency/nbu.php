<?php
/**
* Получение курса валют НБУ с сайта xml.delta-x.com.ua
*
* @package Pilot
* @subpackage Currency
* @version 3.0
...
$DB->query("SHOW TABLES LIKE 'currency_nbu_rate'");

if ($DB->rows != 1) {
	echo "[e] NBU currency module not installed\n";
	exit;
}

if(!$xml_currency = @file_get_contents('http://xml.delta-x.com.ua/nbu.php')) {
...
if (!isset($parsed[0]['_elements']) || !is_array($parsed[0]['_elements']) || count($parsed[0]['_elements']) == 0) {
	echo "[e] NBU currency: XML document doesn't contain currency data\n";
	x($parsed);
	exit;
}
...
while (list(,$row)=each($parsed[0]['_elements'])) {
	...
	$query = "
		REPLACE INTO currency_nbu_rate
		SET
	...
	$query = "
		REPLACE INTO currency_nbu_current
		SET
			currency_id = '".$currency['id']."',
			amount = '".$currency['amount']."',
			rate = '".$currency['rate']."',
