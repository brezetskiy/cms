...
-- Процедуры
--
CREATE DEFINER=`{dbname}`@`%` PROCEDURE `admin_auth`(IN _login CHAR(30), IN _passwd CHAR(32), IN _ip CHAR(15), IN _local_ip CHAR(15), OUT _user_id INTEGER(10))
...
CREATE DEFINER=`{dbname}`@`%` PROCEDURE `build_relation`(IN table_name VARCHAR(64), IN parent_field VARCHAR(64), IN relation_table VARCHAR(64), OUT fetch_rows INTEGER(11))
    SQL SECURITY INVOKER
BEGIN
    DECLARE
           who_has_parents,
...
CREATE DEFINER=`{dbname}`@`%` PROCEDURE `clean_relation`(IN table_name VARCHAR(64), IN id INTEGER(11))
    SQL SECURITY INVOKER
BEGIN
...
CREATE DEFINER=`{dbname}`@`%` PROCEDURE `get_currency_rate`(IN _date DATETIME, IN _currency_from_id SMALLINT, IN _currency_to_id SMALLINT, IN _currency_cross_id SMALLINT, OUT _rate DECIMAL(10,4))
    SQL SECURITY INVOKER
BEGIN
...
CREATE DEFINER=`{dbname}`@`%` PROCEDURE `hosting_get_ip`()
    READS SQL DATA
BEGIN
...
-- Функции
--
CREATE DEFINER=`{dbname}`@`%` FUNCTION `get_age`(birthday DATE, `date` DATE) RETURNS tinyint(4)
    DETERMINISTIC
RETURN
      CASE
...
CREATE DEFINER=`{dbname}`@`%` FUNCTION `html_editor`(id INTEGER(11), table_name VARCHAR(70), field_name VARCHAR(70), title VARCHAR(255)) RETURNS text CHARSET cp1251
    DETERMINISTIC
...
CREATE DEFINER=`{dbname}`@`%` FUNCTION `interval_intersect`(x_from BIGINT, x_to BIGINT, y_from BIGINT, y_to BIGINT) RETURNS tinyint(4)
    DETERMINISTIC
RETURN
      CASE
...
CREATE DEFINER=`{dbname}`@`%` FUNCTION `text_editor`(id INTEGER(11), table_name VARCHAR(70), field_name VARCHAR(70), title VARCHAR(255)) RETURNS text CHARSET cp1251
    DETERMINISTIC
...
CREATE DEFINER=`{dbname}`@`%` FUNCTION `user_check`(user_registered ENUM('true','false'), user_confirmed ENUM('true','false'), user_checked ENUM('true','false'), required_type ENUM('any','registered','confirmed','checked')) RETURNS enum('true','false') CHARSET cp1251
    DETERMINISTIC
    SQL SECURITY INVOKER
return case
