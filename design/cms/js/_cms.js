...
(cmsView = {
	...
	filterBetween: function (element_id, condition) {
		byId(element_id).style.display = (condition == 'between' || condition == 'not between') ? 'inline' : 'none';
	},
	filterActivate: function (element_id) {
		if (byId(element_id+'_condition').value=='') byId(element_id+'_condition').selectedIndex=1;
	},
