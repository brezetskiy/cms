<?php if(empty($this->vars['content'])): ?>
	<a href="/news/rss/" target="_blank"><img src="/img/news/rss_news.gif" border="0"></a>	
	<table class="news_archive">
		<tr>
		<td class="news_date">Категория:</td>
		<td class="news_title"><?php echo $this->vars['type']; ?></td>
	</tr>
		...
		<tr>
			...
			<td class="news_title">
				...
				<?php else: ?>
					<?php echo $this->vars['/news/'][$__key][$_news_key]['headline']; ?>
				<?php endif; ?>
				<div><?php echo $this->vars['/news/'][$__key][$_news_key]['subcontent']; ?></div>
				<?php if(!empty($this->vars['/news/'][$__key][$_news_key]['announcement'])): ?>
					<br><br><span class="comment"><?php echo $this->vars['/news/'][$__key][$_news_key]['announcement']; ?></span>
				<?php endif; ?>
				<div class="comment_count">Комментариев: <?php echo $this->vars['/news/'][$__key][$_news_key]['count_comments']; ?></div>
			</td>
		</tr>
	<?php 
			endwhile;
...
<?php else: ?>
		...
		<div class="post" id="post-<?php echo $this->vars['message_id']; ?>" style="margin-top: 0;">
			<table id="post-head">
				 <tr>
				  	<td id="head-date">
			     		<div class="date">
			     			<span><?php echo $this->vars['month_from']; ?></span> <?php echo $this->vars['day_from']; ?>	
			     		</div>
			     	</td>
			     	<td>
			     	 <div class="title">
	...
	<center>
		<?php if(!empty($this->vars['archive_name'])): ?>
	   		<a href="/news/"><?php echo $this->vars['archive_name']; ?></a>
   		<?php else: ?>
	   		<a href="/news/"><?php echo $this->vars['type_name']; ?></a>
   		<?php endif; ?>
	</center>
	<br />
